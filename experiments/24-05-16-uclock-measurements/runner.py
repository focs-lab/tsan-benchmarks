...
class TestAggStats:
    ...
    uclock_incs_rate: float
    uclock_incs_rate_stdev: float
    uclock_incs_rate_median: float
    uclock_incs_rate_min: float
    uclock_incs_rate_max: float
    ...
    def header():
        return [
            ...
            "uclock_incs_rate",
            "uclock_incs_rate_stdev",
            "uclock_incs_rate_median",
            "uclock_incs_rate_min",
            "uclock_incs_rate_max",
    ...
    def as_row(self):
        return iter(
            [
                ...
                self.uclock_incs_rate,
                self.uclock_incs_rate_stdev,
                self.uclock_incs_rate_median,
                self.uclock_incs_rate_min,
                self.uclock_incs_rate_max,
...
def run_test(test, test_set_name, timeout, report_bugs=True):
    ...
    with subprocess.Popen(BASH_PATH, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=test_env) as process:
        ...
        num_original_acquires = parse_extra_stats("Num original acquires: ", output)
        num_uclock_acquires = parse_extra_stats("Num uclock acquires: ", output)
        num_original_releases = parse_extra_stats("Num original releases: ", output)
        num_uclock_releases = parse_extra_stats("Num uclock releases: ", output)
        num_original_incs = parse_extra_stats("Num original incs: ", output)
        num_uclock_incs = parse_extra_stats("Num uclock incs: ", output)

        uclock_acquires_rate = num_uclock_acquires / num_original_acquires if num_original_acquires > 0 else 0
        uclock_releases_rate = num_uclock_releases / num_original_releases if num_original_releases > 0 else 0
        uclock_incs_rate = num_uclock_incs / num_original_incs if num_original_incs > 0 else 0
        ...
        return TestStats(test_name=test_name,
                         ...
                         uclock_incs_rate=uclock_incs_rate,
...
def aggregate_test_stats(tests_stats: List[TestStats]):
    ...
    uclock_incs_rate_list = list(map(lambda ts: ts.uclock_incs_rate, tests_stats))
    ...
    uclock_incs_rate_mean = statistics.mean(uclock_incs_rate_list)
    uclock_incs_rate_stdev = statistics.stdev(uclock_incs_rate_list)
    uclock_incs_rate_median = statistics.median(uclock_incs_rate_list)
    uclock_incs_rate_min = min(uclock_incs_rate_list)
    uclock_incs_rate_max = max(uclock_incs_rate_list)
    ...
    return TestAggStats(tests_stats[0].test_name,
                        ...
                        uclock_incs_rate=uclock_incs_rate_mean,
                        uclock_incs_rate_stdev=uclock_incs_rate_stdev,
                        uclock_incs_rate_median=uclock_incs_rate_median,
                        uclock_incs_rate_min=uclock_incs_rate_min,
                        uclock_incs_rate_max=uclock_incs_rate_max,
...
def run_tests(test_num_iters: int, timeout: int, report_bugs=True):
    testcases_yaml = yaml.load(open("testcases.yml"), Loader=yaml.FullLoader)
...
def main():
    ...
    for rt in config.runtimes:
        ...
        if not ("report_bugs" in rt.keys() and rt["report_bugs"] == 0):
            ...
            run_tests(rt["iterations"], timeout, False)
